...
export const ProductValidationSchema = z.object({
    ...
    price: z.string({ invalid_type_error: 'Price must be a valid number' }).min(1, { message: "Price must be greater than or equal to 1" }),
    totalQuantity: z.string({ invalid_type_error: 'Total quantity must be a valid number' }).min(1, { message: "Total quantity must be greater than or equal to 1" }),
